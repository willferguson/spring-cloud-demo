...
public class MeatClient {
    ...
    @Autowired
    public MeatClient(@Value("${meatclient.hystrix.timeoutInMilliseconds}") String hystrixTimeout, RestTemplate restTemplate) {
    ...
    public Observable<Meat> getMeat(Meat meat) {
        return Observable.just(restTemplate.postForObject("/meat", meat, Meat.class));
    }
